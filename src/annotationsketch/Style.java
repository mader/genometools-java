...
public class Style {
	...
	GTerror err = new GTerror();
	int rval = GT.INSTANCE.gt_style_load_file(style_ptr, filename, err
		.to_ptr());
	if (rval != 0) {
	    throw new GTerrorJava(err.get_err());
	}
	...
	int rval = GT.INSTANCE.gt_style_load_str(style_ptr, str_obj.to_ptr(),
		err.to_ptr());
	if (rval != 0) {
	    throw new GTerrorJava(err.get_err());
	}
	...
	GT.INSTANCE
		.gt_style_set_color(style_ptr, sect, key, color.getPointer());
    }
